<?php
/**
 * Główny plik indeksowy strony, który ładuje konfiguracje i funkcje wyświetlania
 */
include 'cfg.php';
include 'showpage.php';

// Wyłączenie wyświetlania błędów typu Notice/Warning dla czystszego widoku
error_reporting(E_ALL ^ E_NOTICE ^ E_WARNING);


//Dynamiczne ładowanie stron

// Integracja sklepu i koszyka
include_once 'koszyk.php';

// Obsługa akcji koszyka (dodawanie/usuwanie/update)
if (isset($_POST['akcja'])) {
    if ($_POST['akcja'] == 'add') {
        dodajDoKoszyka($_POST['id_prod'], $_POST['tytul'], $_POST['cena_netto'], $_POST['vat'], $_POST['ilosc']);
        echo "<script>alert('Produkt dodany do koszyka');</script>";
    }
    if ($_POST['akcja'] == 'remove') {
        usunZKoszyka($_POST['nr']);
    }
    if ($_POST['akcja'] == 'update') {
        edytujIlosc($_POST['nr'], $_POST['ilosc']);
    }
}
...
// Sprawdzanie parametru 'idp' i ładowanie odpowiedniej treści
if ($_GET['idp'] == '')
    $strona = pokaz_podstrone(id: 1);
if ($_GET['idp'] == 'glowna')
    $strona = pokaz_podstrone(id: 1);
if ($_GET['idp'] == 'buildings')
    $strona = pokaz_podstrone(id: 2);
if ($_GET['idp'] == 'contact')
    $strona = pokaz_podstrone(id: 5);
if ($_GET['idp'] == 'gallery')
    $strona = pokaz_podstrone(id: 4);
if ($_GET['idp'] == 'jQuery')
    $strona = pokaz_podstrone(id: 6);
if ($_GET['idp'] == 'rankings')
    $strona = pokaz_podstrone(id: 3);

if ($_GET['idp'] == 'sklep') {
    $strona = include 'sklep.php';
}
if ($_GET['idp'] == 'koszyk') {
    $strona = pokazKoszyk();
}
if ($_GET['idp'] == 'films')
    $strona = pokaz_podstrone(id: 7);
?>
...
<body onload="startclock()">
    ...
    <footer>
        <nav>
            <ul>
                <li><a href="index.php?idp=gallery">Galeria zdjęć</a></li>
                <li><a href="index.php?idp=contact">Kontakt</a></li>
            </ul>
        </nav>
